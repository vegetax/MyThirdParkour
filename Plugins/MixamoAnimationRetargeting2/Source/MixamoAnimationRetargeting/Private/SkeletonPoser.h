...
public:
	...
	*/
	...
	@param ForceNewCSBoneNames A set of bone names of Mesh that must be forcefully posed accordingly to their matching bones,
		also if they are the parent of multiple child bones (usually they are not re-posed).
	...
	*/
	void Pose(const USkeletalMesh * Mesh, const FBoneMapper & BoneMapper, const TArray<FName> & PreserveCSBonesNames, const TArray<FName> & ForceNewCSBoneNames, TArray<FTransform> & MeshBonePose) const;

	// Utility methods.
	void PoseBasedOnRigConfiguration(const USkeletalMesh * Mesh, const TArray<FName> & PreserveCSBonesNames, const TArray<FName> & ForceNewCSBoneNames, TArray<FTransform> & MeshBonePose) const;
	void PoseBasedOnCommonBoneNames(const USkeletalMesh * Mesh, const TArray<FName> & PreserveCSBonesNames, const TArray<FName> & ForceNewCSBoneNames, TArray<FTransform> & MeshBonePose) const;
	void PoseBasedOnMappedBoneNames(const USkeletalMesh * Mesh, const TArray<FName> & PreserveCSBonesNames, const TArray<FName> & ForceNewCSBoneNames, const FStaticNamesMapper & SourceToDest_BonesNameMapping, TArray<FTransform> & MeshBonePose) const;
...
private:
	void Pose(const FReferenceSkeleton & EditRefSkeleton, const FBoneMapper & BoneMapper, const TSet<int32> & PreserveCSBonesIndices, const TSet<int32> & ForceNewCSBonesIndices, TArray<FTransform> & MeshBonePoses) const;
