...
) const
{
	...
	TSet<int32> ForceNewCSBonesIndices;
	ForceNewCSBonesIndices.Reserve(ForceNewCSBoneNames.Num());
	for (const FName & BoneName : ForceNewCSBoneNames)
	{
		const int32 BoneIndex = Mesh->GetRefSkeleton().FindBoneIndex(BoneName);
		if (BoneIndex != INDEX_NONE)
		{
			ForceNewCSBonesIndices.Add(BoneIndex);
		}
	}
	...
	Pose(Mesh->GetRefSkeleton(), BoneMapper, PreserveCSBonesIndices, ForceNewCSBonesIndices, MeshBonePose);
	UE_LOG(LogMixamoToolkit, Log, TEXT("END: %s -> %s"), *Mesh->GetName(), *ReferenceSkeleton->GetName());
}



void FSkeletonPoser::PoseBasedOnRigConfiguration(const USkeletalMesh * Mesh, const TArray<FName> & PreserveCSBonesNames, const TArray<FName> & ForceNewCSBoneNames, TArray<FTransform> & MeshBonePose) const
{
	check(Mesh != nullptr);
	Pose(Mesh, FRigConfigurationBoneMapper(& Mesh->GetRefSkeleton(), Mesh->GetSkeleton(), & ReferenceSkeleton->GetReferenceSkeleton(), ReferenceSkeleton), PreserveCSBonesNames, ForceNewCSBoneNames, MeshBonePose);
}



void FSkeletonPoser::PoseBasedOnCommonBoneNames(const USkeletalMesh * Mesh, const TArray<FName> & PreserveCSBonesNames, const TArray<FName> & ForceNewCSBoneNames, TArray<FTransform> & MeshBonePose) const
{
	check(Mesh != nullptr);
	Pose(Mesh, FEqualNameBoneMapper(& Mesh->GetRefSkeleton(), & ReferenceSkeleton->GetReferenceSkeleton()), PreserveCSBonesNames, ForceNewCSBoneNames, MeshBonePose);
}
...
{
	check(Mesh != nullptr);
	Pose(Mesh, FNameTranslationBoneMapper(& Mesh->GetRefSkeleton(), & ReferenceSkeleton->GetReferenceSkeleton(), SourceToDest_BonesNameMapping), PreserveCSBonesNames, ForceNewCSBoneNames, MeshBonePose);
}
...
void FSkeletonPoser::Pose(
	...
	const TSet<int32> & ForceNewCSBonesIndices,
...
) const
{
	...
	for (int32 EditBoneIndex = 0; EditBoneIndex < NumBones; ++EditBoneIndex)
	{
		...
		if (EditChildrens[EditBoneParentIndex] > 1 && !ForceNewCSBonesIndices.Contains(EditBoneParentIndex))
		{
			...
			UE_LOG(LogMixamoToolkit, Log, TEXT("Bone %d (%s) not re-oriented because its parent bone (%d - %s) controls also other bones"), EditBoneIndex, *EditRefSkeleton.GetBoneName(EditBoneIndex).ToString(), EditBoneParentIndex, *EditRefSkeleton.GetBoneName(EditBoneParentIndex).ToString());
			continue;
		}
